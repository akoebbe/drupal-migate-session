...
(function () {
    ...
    while (src) {
      ...
      if (top && (cap = this.rules.table.exec(src))) {
        ...
        for (i = 0; i < item.align.length; i++)if (/^ *-+: *$/.test(item.align[i]))item.align[i] = "right"; else if (/^ *:-+: *$/.test(item.align[i]))item.align[i] =
          "center"; else if (/^ *:-+ *$/.test(item.align[i]))item.align[i] = "left"; else item.align[i] = null;
  ...
  InlineLexer.prototype.outputLink = function (cap, link) {
    if (cap[0][0] !== "!")return'<a href="' + escape(link.href) + '"' + (link.title ? ' title="' + escape(link.title) + '"' : "") + ">" + this.output(cap[1]) + "</a>"; else return'<img src="' + escape(link.href) + '" alt="' + escape(cap[1]) + '"' + (link.title ? ' title="' +
      escape(link.title) + '"' : "") + ">"
  };
  ...
  marked.parse = marked;
  if (typeof exports === "object")module.exports = marked; else if (typeof define === "function" && define.amd)define(function () {
    return marked
